...
- The app stores the Thread dataset in encrypted app-private storage and persists openHAB/OTBR configuration.
- The app can check openHAB REST readiness, require an online Matter controller Thing, observe Inbox state, and watch Inbox SSE events.
...
| Basic OTBR connectivity check | Implemented as generic HTTP reachability | `HttpOtbrClient`, `MainActivity.checkOtbrConnectivity()`, `docs/implementation-status.md` |
...
| openHAB Matter readiness and Inbox confirmation | Implemented | `HttpOpenHabClient`, `HttpOpenHabInboxClient`, `OpenHabInboxSseClient` |
