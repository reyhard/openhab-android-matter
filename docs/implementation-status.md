...
- The Compose advanced troubleshooting screen exposes one-tap OpenCommissioningWindow retry for the currently staged bootstrap device and a forget-from-phone action that attempts connectedhomeip unpair before clearing this app's stored bootstrap state. Failed unpair attempts keep the staged entry and expose a force-remove action that only clears this app's local state.
- The Compose settings flow has a dedicated **Devices on this phone** submenu for the app-stored bootstrap device, with visible back navigation, expandable staging diagnostics, OpenCommissioningWindow retry, details navigation, connectedhomeip unpair, and force-remove local cleanup after unpair failure. This is currently a view of this app's persisted staging state, not a full connectedhomeip fabric inventory.
