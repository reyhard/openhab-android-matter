...
- The Compose settings flow exposes a dedicated **Devices on this phone** submenu for the currently staged bootstrap device. From there, users can expand local staging diagnostics, retry OpenCommissioningWindow and continue the openHAB scan/Inbox handoff, open a details screen, or forget the staged device from the phone. Forget first selects the connectedhomeip controller through the native readiness gate and calls native unpair for the stored node id; only a successful unpair clears this app's stored bootstrap state. If unpair fails, the staged entry remains visible and a force-remove action can clear only this app's local state. This submenu reflects the app's persisted bootstrap state only; full connectedhomeip fabric enumeration is still future work.
